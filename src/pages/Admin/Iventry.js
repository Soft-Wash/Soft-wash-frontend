...
const status2 = "pending"
  ...
  const MonthlyEarnings = () => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/order/paymentstatus/completed?payment_status=${status}`)
    .then((resp)=>{
      setPaymentStatus(resp.data)
      const sortedOrders = resp.data.map((price)=> price.subtotal)
      const filteredUndefined = sortedOrders.filter((subtotal)=>subtotal !== undefined)
  ...
  const monthBefore=()=>{
    axios.get(`${process.env.REACT_APP_BASE_URL}/order/monthbefore`)
    .then((resp)=>{
      const sortedOrders = resp.data.map((price)=> price.subtotal)
      const filteredUndefined = sortedOrders.filter((subtotal)=>subtotal !== undefined)
      const total = filteredUndefined.reduce((acc,curr)=> acc + curr,0)
  ...
  const lastMonthSales=()=>{
    axios.get(`${process.env.REACT_APP_BASE_URL}/order/month`)
    .then((resp)=>{
      const totalSales = resp.data.map((sales)=>  sales.subtotal)
      const filteredUndefined = totalSales.filter((subtotal)=>subtotal !== undefined)
      const total = filteredUndefined.reduce((acc,curr)=> acc + curr,0)
      setMonth(total)
    })
  }
  ...
  const DeclinedOrders=()=>{
    axios.get(`${process.env.REACT_APP_BASE_URL}/order/status?status=${status2}`)
    .then((resp)=>{
          ...
          <div className="category-container">
            <div className="icon-container mb-3">
              ...
              <div className="state-percent-divv">
                ...
                <p className="state-percent-divv-p">{Math.round(growthPercentage * 100) / 100}%</p>
              </div>
            </div>
            <div className="icon-container mb-3">
              <div className="icon-container-innerd1">
